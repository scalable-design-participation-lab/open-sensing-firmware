...
#define productUID "product:edu.mit.rkovacs:neudeployed"
#define PCAADDR 0x70
...
void setup() {
  ...
  // ---------- Initialize SEN55 ----------
  pcaselect(SEN55_CHANNEL);
  sen5x.begin(Wire);
  uint16_t sen55Error;
  char errorMessage[256];
  sen55Error = sen5x.deviceReset();
  if (sen55Error) {
...
void loop() {
  ...
  if (scd4x.readMeasurement()) {
    scdCO2 = scd4x.getCO2();
    scdTemp = scd4x.getTemperature();
    scdHumidity = scd4x.getHumidity();

    Serial.println("SCD41 Values:");
    Serial.print("scdCO2 (ppm): ");
    Serial.println(scdCO2);
    Serial.print("scdTemp (°C): ");
    Serial.println(scdTemp);
    Serial.print("scdHumidity (%): ");
    Serial.println(scdHumidity);
  } else {
